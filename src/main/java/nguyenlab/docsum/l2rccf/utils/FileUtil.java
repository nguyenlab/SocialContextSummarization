...
import nguyenlab.docsum.l2rccf.entities.Sentence;
import nguyenlab.docsum.l2rccf.entities.Document;

public class FileUtil {

    private String regexspace = " ";
    private String regextab = "\t";
    static List<String> stop_words = null;
    String stopword = "dict/stopwords_en.txt";

    public void loadStopwords(String file) throws IOException {
        stop_words = readFileString(file);
        System.out.println(stop_words.size());
        System.out.println("Successful reading stopword file");
    }
    ...
    public Map<String, Double> readLDAFile(String pathFile) throws IOException {
        Map<String, Double> result = new HashMap<String, Double>();
        FileReader read = new FileReader(pathFile.replace(",", ""));
        BufferedReader in = new BufferedReader(read);
        String s = "";
        while ((s = in.readLine()) != null) {
            String[] arr = s.toLowerCase().split("\\s+");
    ...
    public List<Sentence> readFileSent(String file) throws IOException {
        List<Sentence> array = new ArrayList<Sentence>();
        FileReader read = new FileReader(file);
        File f = new File(file);
        if (!f.exists()) {
            read.close();
