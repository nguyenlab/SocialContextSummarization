...
/**
 * An Implementation of the LexRank algorithm described in the paper "LexRank:
 * Graph-based Centrality as Salience in Text Summarization", Erkan & Radev '04,
...
public class LexRank {
     ...
     * scores are used. The paper authors note that non-continuous LexRank seems
     * to perform better.
     */
    public static <T extends Similar<T>> LexRankResult<T> rank(List<T> data,
            double similarityThreshold, boolean continuous) {
        LexRankResult<T> results = new LexRankResult<T>();
        if (data.size() == 0) {
            return results;
        }
        double[][] similarities = similarityMatrix(data);
        double[][] transitionProbabilities = transitionProbabilities(similarities, similarityThreshold,
                continuous);
        ...
        double[] rankings = powerIteration(transitionProbabilities, data.size(), 0.001, 100);
    ...
    /**
     * Computes a similarity matrix given a set of data. Assumes that the
     ...
     * random walker on a graph to go from any one node to any other node, where
     * all edges come from sufficiently high similarities between nodes.
     */
     ...
     */
    private static double[] powerIteration(double[][] stochasticMatrix, int size, double epsilon,
            int maxIterations) {
